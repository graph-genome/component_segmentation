...
def segment_matrix(matrix: List[Path]):
    ...
    for path in matrix:
        for upstream, downstream in path.links:  # Links are generated by odgi based
            ...
            if downstream < upstream or any([i in path for i in missing_range if i > 0]):
                dividers.add(upstream + 1)  # the first position of the new component
