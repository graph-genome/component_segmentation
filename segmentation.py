...
def find_dividers(matrix: List[Path]) -> Tuple[Dict[int, Dict[int, set]],
    ...
    # uniq_links = {all links where
    #                   downstream >= upstream and
    #                   there's a path containing both the link and a bin with id in the range [upstream + 1, downstream - 1]
    #              }
    ...
    for i, path in enumerate(matrix):
        ...
        # TODO:
        # - run numpy.searchsorted(links, bin ids)
        # - identify links that have desired overlaps
        # - add them to uniq_links

        for link in links:  # Links are generated by odgi based
            upstream, downstream = int(link[0]), int(link[1])
